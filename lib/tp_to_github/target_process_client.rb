...
module TpToGithub
  class TargetProcessClient
    ...
    # NEW: Fetch assignment records for a user story (returns array of assignment hashes)
    def assignments_for_user_story(story_id:)
      response = connection.get("/api/v1/Assignments") do |req|
        req.params["where"] = "Assignable.Id eq #{story_id}"
        req.params["select"] = "Id,GeneralUser.Login"
      end
